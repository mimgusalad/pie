...
const getCreateGroupChannel = state => params => new Promise((resolve, reject) => {
  ...
  sdk.groupChannel.createChannel(params).then(channel => {
    resolve(channel);
    pubSub.publish(topics.PUBSUB_TOPICS.CREATE_CHANNEL, {
      channel
    });
  }).catch(reject);
});
...
class UikitMessageHandler {
  constructor() {
    _rollupPluginBabelHelpers._defineProperty(this, "_onPending", utils.noop);
    _rollupPluginBabelHelpers._defineProperty(this, "_onFailed", utils.noop);
    _rollupPluginBabelHelpers._defineProperty(this, "_onSucceeded", utils.noop);
  }
...
const getDeleteMessage = state => (channel, message) => new Promise((resolve, reject) => {
  ...
  channel.deleteMessage(message).then(() => {
    pubSub.publish(topics.PUBSUB_TOPICS.DELETE_MESSAGE, {
      messageId,
      channel
    });
    resolve(message);
