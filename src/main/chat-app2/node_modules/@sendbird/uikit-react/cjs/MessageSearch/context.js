    ...
    if (sdk && channelUrl && sdk.createMessageSearchQuery && currentChannel) {
      if (requestString) {
        currentChannel.refresh().then(channel => {
          const inputSearchMessageQueryObject = _rollupPluginBabelHelpers._objectSpread2({
...
function useSearchStringEffect(_ref, _ref2) {
  ...
  const [requestString, setRequestString] = React.useState('');
  const [debouncingTimer, setDebouncingTimer] = React.useState(null);
  React.useEffect(() => {
...
const MessageSearchContext = /*#__PURE__*/React__default["default"].createContext(undefined);
const MessageSearchProvider = props => {
  ...
  // hook variables
  const [retryCount, setRetryCount] = React.useState(0); // this is a trigger flag for activating useGetSearchMessages
  const [selectedMessageId, setSelectedMessageId] = React.useState(0);
  const [messageSearchStore, messageSearchDispatcher] = React.useReducer(reducer, initialState);
  const {
    allMessages,
    loading,
    isInvalid,
  ...
  return /*#__PURE__*/React__default["default"].createElement(MessageSearchContext.Provider, {
    value: {
      channelUrl,
      searchString,
      requestString,
...
const useMessageSearchContext = () => React__default["default"].useContext(MessageSearchContext);
